...
const buildFieldStyles = (params: CanvasFieldsValues, hasValue: boolean): CSSProperties => ({
  display: "flex",
...
const buildDropdownContainerStyles = (params: CanvasFieldsValues): CSSProperties => ({
  listStyle: "none",
  border:
...
const buildDropdownOptionStyles = (params: CanvasFieldsValues): CSSProperties => ({
  width: "100%",
  border: "none",
  cursor: "pointer",
  backgroundColor:
...
export default function CanvasSelectField({ params, onChange, value }: Props) {
  ...
  const fieldStyles = buildFieldStyles(params, hasValue);
  const dropdownContainerStyles = buildDropdownContainerStyles(params);
  const dropdownOptionStyles = buildDropdownOptionStyles(params);
  ...
  return (
    <div style={wrapperStyles}>
      ...
      {Array.isArray(params?.options) && params?.options?.length > 0 && open && (
